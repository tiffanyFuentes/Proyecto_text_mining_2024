...
def aplicar_prompts_y_seleccionar(texto, prompts, llama_pipeline):
    outputs = []
    for i, prompt in enumerate(prompts, 1):
        entrada = f"{prompt}\n\nTexto judicial:\n{texto}"
        resultado = llama_pipeline(entrada, max_new_tokens=500, num_return_sequences=1)
        resumen_generado = resultado[0]["generated_text"].strip()
        outputs.append((f"Prompt {i}", resumen_generado))

    print("\nResultados generados:\n")
    for i, (prompt_name, output) in enumerate(outputs):
        print(f"\n{prompt_name}:\n{output}\n")

    seleccion = int(input("Seleccione el número del resultado que prefiera (1, 2 o 3): ")) - 1
    return outputs[seleccion][1]
...
def main():
    ...
    """Eres un asistente legal que debe extraer los datos principales de una sentencia judicial. Los únicos datos que debes obtener son los siguientes:
    ...
    Ejemplo de los datos de una causa:
    ...
    prompts_sintesis = [
    """Eres un asistente legal que debe crear una síntesis concisa
    ...
    prompts_sumarios = [
    """Eres un asistente legal que debe a partir del siguiente documento judicial, generar un **sumario jurisprudencial** que capture la doctrina clave del fallo. El sumario debe cumplir con las siguientes pautas:
    ...
    **SUMARIO:**
    ...
    Aqui esta el texto judicial:
    """,
    """Eres un asistente legal que debe generar un sumario jurisprudencial a partir del siguiente documento judicial, capturando la doctrina clave del fallo. El sumario debe cumplir con las siguientes pautas:
    ...
    Ejemplo correcto de un resumen con varios sumarios:

    SUMARIO:

    INCIDENTE DE NULIDAD DE CÉDULA.
    Cómputo del plazo. E-CÉDULA. Servicio Extranet: Barandilla Virtual - Mis eCédulas Recibidas. Obligación del letrado de revisar la Barandilla Virtual. Acuerdo Reglamentario, TSJ, Serie "A" 1103/12.
    El art 78 del CPC exige la articulación del incidente de nulidad dentro de los cinco días de conocido el “acto viciado”. Este plazo se computa desde que el interesado toma conocimiento del acto viciado por cualquiera de los medios de notificación contemplados por la ley ritual. En las notificaciones mediante e-cédula, la comunicación queda disponible en el servicio Extranet (Barandilla Virtual - Mis eCédulas Recibidas), del sitio oficial del Poder Judicial de Córdoba, y es responsabilidad de los letrados consultarlo periódicamente (art. 4 del Acuerdo Reglamentario TSJ “A” 1103/12).

    SUMARIO:

    PLAZOS PROCESALES.
    E-CÉDULA. Notificación enviada durante la feria judicial. Servicio Extranet: Barandilla Virtual - Mis eCédulas Recibidas. Reanudación de la actividad tribunalicia. Validez de la cédula.
    Obligación del letrado de revisar la Barandilla Virtual. Mensaje de cortesía: duración. Como en el campo “Mis Novedades” del servicio Extranet (Barandilla Virtual- Mis eCédulas) la cuestión se mantiene por cinco días corridos, las notificaciones concretadas durante el receso judicial que los letrados no vieron ese día, tampoco figuran en “Mis Novedades” al momento de la reanudación de la actividad tribunalicia. Esto implica que el aviso de cortesía no reemplaza la notificación en “Mis eCédulas Recibidas”, que debe revisarse regularmente.

    SUMARIO:

    PLAZOS PROCESALES.
    E-CÉDULA. Servicio Extranet: Barandilla Virtual - Mis eCédulas Recibidas. Alcance de la responsabilidad del letrado.
    Las notificaciones que se encuentran en “Mis eCédulas Recibidas” equivalen a la cédula papel que debe ser revisada al reintegrarse a la actividad judicial. Al estar las novedades solo por 5 días corridos, existe el riesgo de que algún decreto quede notificado según lo establece el art. 153 CPC. No es el aviso de cortesía lo que determina la vigencia de los plazos, sino la existencia de la actuación judicial accesible en el servicio Extranet.

    SUMARIO:

    PLAZOS PROCESALES.
    Servicio Extranet: Barandilla Virtual - Mis eCédulas Recibidas.
    El servicio de novedades brindado durante cinco días no sustituye a "la barandilla virtual" donde se encuentran las notificaciones oficiales. El letrado debe consultar en el servicio Extranet para verificar la existencia de cédulas, aun cuando no aparezcan en la bandeja de novedades.

    ------------

    Aquí está el texto judicial:
    """,
    """Eres un asistente legal encargado de generar un sumario jurisprudencial basado en el siguiente documento judicial. Céntrate en capturar la doctrina clave según las siguientes pautas:

    1. **Doctrina única**: Aborda solo una cuestión jurídica, evitando mezclar temas.
    2. **Autonomía**: El sumario debe ser claro y comprensible sin el texto completo.
    3. **Abstracción**: Excluye detalles específicos (como nombres y fechas), a menos que sean esenciales para la doctrina.
    4. **Claridad**: Redacta de manera precisa y concisa.
    5. **Fidelidad**: Refleja el sentido original de la sentencia sin añadir interpretaciones.
    ...
    **Errores en el ejemplo incorrecto**:
    ...
    Documento judicial a resumir:"""
    ]

    # Aplicar prompts y obtener selección del usuario
    datos_seleccionado = aplicar_prompts_y_seleccionar(contenido, prompts_datos, llama_pipeline)
    sintesis_seleccionado = aplicar_prompts_y_seleccionar(contenido, prompts_sintesis, llama_pipeline)
    sumarios_seleccionado = aplicar_prompts_y_seleccionar(contenido, prompts_sumarios, llama_pipeline)

    # Crear documento final
    documento_final = (
        f"DATOS:\n{datos_seleccionado}\n\n"
        f"SÍNTESIS:\n{sintesis_seleccionado}\n\n"
        f"SUMARIOS:\n{sumarios_seleccionado}\n"
    )

    # Guardar en un archivo
    output_path = archivo_seleccionado.replace(".txt", "_resumen_llama.txt")
